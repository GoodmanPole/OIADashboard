...
function getLocations(data) {
    ...
    $('#intl_briefing tbody').fadeOut(400, function () {
        $('#intl_briefing tbody').empty();
        ...
        $.each(data.locations, function (i, locations) {
			...
			if (display == true && locations.country == 'Germany') {
				var m = '<tr'; 
				if (results % 2 == 0){
					m += ' class="stripe"><td>'
				} else {m += '><td>'};
        ...
        if (results == 0) {
            $('#intl_briefing tbody').append('<tr><td colspan="3">No results found.</td></tr>');
        }
        $('#intl_briefing tbody').fadeIn();
    });
}
function decode(str) {
